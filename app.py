#!/usr/bin/env python3
"""
Self-Storage Facility Scanner - Web Application
Systematically browse areas to identify self-storage facilities
"""
...
<head>
    ...
    <style>
        ...
        .maps-frame { 
            width: 100%; height: 500px; border: 2px solid #dee2e6; 
    ...
    <div class="container">
        ...
        <div class="section hidden" id="scanning-section">
            ...
            <div class="current-location" id="current-location">
                <strong>📍 Current Location:</strong> <span id="location-coords">Scanning not started</span><br>
                <strong>📊 Position:</strong> <span id="location-index">0 / 0</span>
            </div>
            
            <!-- Embedded Google Maps -->
            <iframe id="maps-frame" class="maps-frame" src="" allowfullscreen="" loading="lazy" 
                    title="Google Maps showing current scanning location"></iframe>
            ...
            try {
                ...
                if (result.success) {
                    if (result.completed) {
                        ...
                        const point = result.current_point;
                        ...
                        document.getElementById('location-coords').textContent = `${point.lat}, ${point.lon}`;
                        document.getElementById('location-index').textContent = `${result.current_index + 1} / ${result.total_points}`;
                        
                        // Update Google Maps iframe
                        document.getElementById('maps-frame').src = point.google_maps_url;
                    }
                    updateStats();
                }
            } catch (error) {
                        ...
                        div.innerHTML = `
                            <div style="display: flex; justify-content: between; align-items: flex-start;">
                                <div style="flex-grow: 1;">
                                    ...
                                    <strong>🌍 Coordinates:</strong> ${bookmark.lat}, ${bookmark.lon}<br>
                                    <strong>🕒 Time:</strong> ${new Date(bookmark.timestamp).toLocaleString()}<br>
                                    <div style="margin-top: 10px;">
                                        <a href="${bookmark.google_maps_url}" target="_blank">🗺️ View in Google Maps</a>
